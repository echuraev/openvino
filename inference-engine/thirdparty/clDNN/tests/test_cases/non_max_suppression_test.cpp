...
TYPED_TEST(non_max_suppression_basic, soft_nms_sigma) {
    ...
    topo.add(non_max_suppression("nms", "boxes", "scores", 6, false, "num_per_class", "iou_threshold", "score_threshold", "soft_nms_sigma"));
    ...
    size_t offset = 3;
    for (size_t i = 0; i < out_ptr.size() / offset; ++i) {
        std:: cout << out_ptr[offset * i] << " " << out_ptr[offset * i + 1] << " " << out_ptr[offset * i + 2] << std::endl;
    }
